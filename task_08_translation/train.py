"""
train.py — Task 08: Traduzione Automatica EN-IT
Uso: python train.py [--dry_run] [--model_size 4b] [--dataset ...] [--output_dir ...] [--num_epochs N] [--batch_size N]
"""
...
DEFAULT_MODEL   = "4b"
DEFAULT_DATASET = "Helsinki-NLP/europarl-corpus"
DEFAULT_OUT_DIR = "./checkpoints/task_08_translation"
MAX_SEQ_LENGTH  = 512
...
def row_to_prompt(row: dict) -> str:
    ...
    user_text = str(row.get("translation", "")).strip()
    label_raw = row.get("translation", "")
    label_str = str(label_raw).strip()
    if SYSTEM_PROMPT:
        return (
            "<start_of_turn>user\n" + SYSTEM_PROMPT + "\n\n"
            "Testo in inglese: " + user_text + "\n"
            "<end_of_turn>\n<start_of_turn>model\n" + label_str + "\n<end_of_turn>"
        )
    return (
        "<start_of_turn>user\nTesto in inglese: " + user_text + "\n"
...
def train(args):
    ...
    logger.info("Avvio training Task 08 — Traduzione Automatica EN-IT...")
    trainer.train()
...
def parse_args():
    p = argparse.ArgumentParser(description="Task 08 — Traduzione Automatica EN-IT")
