"""
train.py — Task 12: Text Style Transfer
...
DEFAULT_MODEL   = "4b"
DEFAULT_DATASET = "s-nlp/paradetox"
DEFAULT_OUT_DIR = "./checkpoints/task_12_style_transfer"
MAX_SEQ_LENGTH  = 512
...
def row_to_prompt(row: dict) -> str:
    ...
    user_text = str(row.get("en_toxic_comment", "")).strip()
    label_raw = row.get("en_neutral_comment", "")
    label_str = str(label_raw).strip()
    if SYSTEM_PROMPT:
        return (
            "<start_of_turn>user\n" + SYSTEM_PROMPT + "\n\n"
            "Testo tossico: " + user_text + "\n"
            "<end_of_turn>\n<start_of_turn>model\n" + label_str + "\n<end_of_turn>"
        )
    return (
        "<start_of_turn>user\nTesto tossico: " + user_text + "\n"
...
def train(args):
    ...
    logger.info("Avvio training Task 12 — Text Style Transfer...")
    trainer.train()
...
def parse_args():
    p = argparse.ArgumentParser(description="Task 12 — Text Style Transfer")
