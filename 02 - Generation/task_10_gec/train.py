"""
train.py — Task 10: Grammatical Error Correction
...
TASK_NAME       = "gec"
DEFAULT_MODEL   = "4b"
DEFAULT_DATASET = "grammarly/coedit"
DEFAULT_OUT_DIR = "./checkpoints/task_10_gec"
MAX_SEQ_LENGTH  = 512
...
def row_to_prompt(row: dict) -> str:
    ...
    user_text = str(row.get("src", "")).strip()
    label_raw = row.get("tgt", "")
    label_str = str(label_raw).strip()
    if SYSTEM_PROMPT:
        return (
            "<start_of_turn>user\n" + SYSTEM_PROMPT + "\n\n"
            "Testo con errori: " + user_text + "\n"
            "<end_of_turn>\n<start_of_turn>model\n" + label_str + "\n<end_of_turn>"
        )
    return (
        "<start_of_turn>user\nTesto con errori: " + user_text + "\n"
...
def train(args):
    ...
    logger.info("Avvio training Task 10 — Grammatical Error Correction...")
    trainer.train()
...
def parse_args():
    p = argparse.ArgumentParser(description="Task 10 — Grammatical Error Correction")
