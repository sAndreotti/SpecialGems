"""
train.py — Task 21: Instruction Following / SFT
...
TASK_NAME       = "sft"
DEFAULT_MODEL   = "4b"
DEFAULT_DATASET = "mchl-labs/stambecco_data_it"
DEFAULT_OUT_DIR = "./checkpoints/task_21_sft"
MAX_SEQ_LENGTH  = 2048
SYSTEM_PROMPT   = """"""
LABEL_MAP       = {}
...
def row_to_prompt(row: dict) -> str:
    ...
    user_text = str(row.get("instruction", "")).strip()
    label_raw = row.get("output", "")
    label_str = str(label_raw).strip()
    if SYSTEM_PROMPT:
        return (
            "<start_of_turn>user\n" + SYSTEM_PROMPT + "\n\n"
            "Istruzione: " + user_text + "\n"
            "<end_of_turn>\n<start_of_turn>model\n" + label_str + "\n<end_of_turn>"
        )
    return (
        "<start_of_turn>user\nIstruzione: " + user_text + "\n"
...
def train(args):
    ...
    logger.info("Avvio training Task 21 — Instruction Following / SFT...")
    trainer.train()
...
def parse_args():
    p = argparse.ArgumentParser(description="Task 21 — Instruction Following / SFT")
