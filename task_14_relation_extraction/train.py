"""
train.py — Task 14: Relation Extraction
...
DEFAULT_MODEL   = "4b"
DEFAULT_DATASET = "thunlp/fewrel"
DEFAULT_OUT_DIR = "./checkpoints/task_14_relation_extraction"
MAX_SEQ_LENGTH  = 512
...
def row_to_prompt(row: dict) -> str:
    ...
    user_text = str(row.get("tokens", "")).strip()
    label_raw = row.get("relation", "")
    label_str = str(label_raw).strip()
    if SYSTEM_PROMPT:
        return (
            "<start_of_turn>user\n" + SYSTEM_PROMPT + "\n\n"
            "Testo con entità marcate: " + user_text + "\n"
            "<end_of_turn>\n<start_of_turn>model\n" + label_str + "\n<end_of_turn>"
        )
    return (
        "<start_of_turn>user\nTesto con entità marcate: " + user_text + "\n"
...
def train(args):
    ...
    logger.info("Avvio training Task 14 — Relation Extraction...")
    trainer.train()
...
def parse_args():
    p = argparse.ArgumentParser(description="Task 14 — Relation Extraction")
