"""
train.py — Task 19: Structured Data Extraction
...
DEFAULT_MODEL   = "4b"
DEFAULT_DATASET = "paraloq/json_data_extraction"
DEFAULT_OUT_DIR = "./checkpoints/task_19_structured_extraction"
MAX_SEQ_LENGTH  = 1024
...
def row_to_prompt(row: dict) -> str:
    ...
    user_text = str(row.get("input", "")).strip()
    label_raw = row.get("output", "")
    label_str = str(label_raw).strip()
    if SYSTEM_PROMPT:
        return (
            "<start_of_turn>user\n" + SYSTEM_PROMPT + "\n\n"
            "Testo e Schema JSON: " + user_text + "\n"
            "<end_of_turn>\n<start_of_turn>model\n" + label_str + "\n<end_of_turn>"
        )
    return (
        "<start_of_turn>user\nTesto e Schema JSON: " + user_text + "\n"
...
def train(args):
    ...
    logger.info("Avvio training Task 19 — Structured Data Extraction...")
    trainer.train()
...
def parse_args():
    p = argparse.ArgumentParser(description="Task 19 — Structured Data Extraction")
