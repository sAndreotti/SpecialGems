"""
train.py — Task 11: Text Simplification
...
DEFAULT_MODEL   = "4b"
DEFAULT_DATASET = "wiki_auto"
DEFAULT_OUT_DIR = "./checkpoints/task_11_simplification"
MAX_SEQ_LENGTH  = 512
...
def row_to_prompt(row: dict) -> str:
    ...
    user_text = str(row.get("source", "")).strip()
    label_raw = row.get("target", "")
    label_str = str(label_raw).strip()
    if SYSTEM_PROMPT:
        return (
            "<start_of_turn>user\n" + SYSTEM_PROMPT + "\n\n"
            "Testo complesso: " + user_text + "\n"
            "<end_of_turn>\n<start_of_turn>model\n" + label_str + "\n<end_of_turn>"
        )
    return (
        "<start_of_turn>user\nTesto complesso: " + user_text + "\n"
...
def train(args):
    ...
    logger.info("Avvio training Task 11 — Text Simplification...")
    trainer.train()
...
def parse_args():
    p = argparse.ArgumentParser(description="Task 11 — Text Simplification")
