"""
train.py — Task 16: Code Generation
...
DEFAULT_OUT_DIR = "./checkpoints/task_16_code_generation"
MAX_SEQ_LENGTH  = 1024
...
def row_to_prompt(row: dict) -> str:
    ...
    user_text = str(row.get("text", "")).strip()
    label_raw = row.get("code", "")
    label_str = str(label_raw).strip()
    if SYSTEM_PROMPT:
        return (
            "<start_of_turn>user\n" + SYSTEM_PROMPT + "\n\n"
            "Specifica funzione: " + user_text + "\n"
            "<end_of_turn>\n<start_of_turn>model\n" + label_str + "\n<end_of_turn>"
        )
    return (
        "<start_of_turn>user\nSpecifica funzione: " + user_text + "\n"
...
def train(args):
    ...
    logger.info("Avvio training Task 16 — Code Generation...")
    trainer.train()
...
def parse_args():
    p = argparse.ArgumentParser(description="Task 16 — Code Generation")
