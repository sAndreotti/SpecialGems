...
TASK_ID         = "15"
TASK_NAME       = "text_to_sql"
DEFAULT_MODEL   = "4b"
DEFAULT_DATASET = "xlangai/spider"
DEFAULT_OUT_DIR = "./checkpoints/task_15_text_to_sql"
MAX_SEQ_LENGTH  = 512
...
def row_to_prompt(row: dict) -> str:
    ...
    user_text = str(row.get("question", "")).strip()
    label_raw = row.get("query", "")
    label_str = str(label_raw).strip()
    if SYSTEM_PROMPT:
        return (
            "<start_of_turn>user\n" + SYSTEM_PROMPT + "\n\n"
            "Domanda e Schema DB: " + user_text + "\n"
            "<end_of_turn>\n<start_of_turn>model\n" + label_str + "\n<end_of_turn>"
        )
    return (
        "<start_of_turn>user\nDomanda e Schema DB: " + user_text + "\n"
...
def train(args):
    ...
    logger.info("Avvio training Task 15 — Text-to-SQL...")
    trainer.train()
...
def parse_args():
    p = argparse.ArgumentParser(description="Task 15 — Text-to-SQL")
