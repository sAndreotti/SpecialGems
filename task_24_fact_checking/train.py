"""
train.py — Task 24: Fact Checking
...
DEFAULT_MODEL   = "4b"
DEFAULT_DATASET = "fever/fever"
DEFAULT_OUT_DIR = "./checkpoints/task_24_fact_checking"
MAX_SEQ_LENGTH  = 1024
...
def row_to_prompt(row: dict) -> str:
    ...
    user_text = str(row.get("claim", "")).strip()
    label_raw = row.get("label", "")
    label_str = LABEL_MAP.get(int(label_raw) if str(label_raw).isdigit() else label_raw, str(label_raw))
    if SYSTEM_PROMPT:
        return (
            "<start_of_turn>user\n" + SYSTEM_PROMPT + "\n\n"
            "Claim ed Evidenza: " + user_text + "\n"
            "<end_of_turn>\n<start_of_turn>model\n" + label_str + "\n<end_of_turn>"
        )
    return (
        "<start_of_turn>user\nClaim ed Evidenza: " + user_text + "\n"
...
def train(args):
    ...
    logger.info("Avvio training Task 24 — Fact Checking...")
    trainer.train()
...
def parse_args():
    p = argparse.ArgumentParser(description="Task 24 — Fact Checking")
