"""
dataset_prep.py — Task 07: Question Answering
Prepara il dataset crux82/squad_it per il fine-tuning di Gemma 3 1B.
"""
...
SYSTEM_PROMPT = """Rispondi alla domanda basandoti sul contesto fornito. Se la risposta non è nel contesto, rispondi 'Non so'."""
LABEL_MAP = {}
...
def make_prompt(user_text: str, label_str: str) -> str:
    if SYSTEM_PROMPT:
        return (
            "<start_of_turn>user\n" + SYSTEM_PROMPT + "\n\n"
            "Contesto e Domanda: " + user_text.strip() + "\n"
            "<end_of_turn>\n<start_of_turn>model\n" + label_str.strip() + "\n<end_of_turn>"
        )
    return (
        "<start_of_turn>user\nContesto e Domanda: " + user_text.strip() + "\n"
...
def prepare_dataset(save_path: str = "./data/task_07") -> DatasetDict:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Prepara dataset Task 07 — Question Answering")
    parser.add_argument("--save_path", default="./data/task_07")
