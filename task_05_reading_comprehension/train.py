"""
train.py — Task 05: Reading Comprehension
...
DEFAULT_MODEL   = "4b"
DEFAULT_DATASET = "ehovy/race"
DEFAULT_OUT_DIR = "./checkpoints/task_05_reading_comprehension"
MAX_SEQ_LENGTH  = 1024
...
def row_to_prompt(row: dict) -> str:
    ...
    user_text = str(row.get("article", "")).strip()
    label_raw = row.get("answer", "")
    label_str = str(label_raw).strip()
    if SYSTEM_PROMPT:
        return (
            "<start_of_turn>user\n" + SYSTEM_PROMPT + "\n\n"
            "Brano, Domanda e Opzioni: " + user_text + "\n"
            "<end_of_turn>\n<start_of_turn>model\n" + label_str + "\n<end_of_turn>"
        )
    return (
        "<start_of_turn>user\nBrano, Domanda e Opzioni: " + user_text + "\n"
...
def train(args):
    ...
    logger.info("Avvio training Task 05 — Reading Comprehension...")
    trainer.train()
...
def parse_args():
    p = argparse.ArgumentParser(description="Task 05 — Reading Comprehension")
